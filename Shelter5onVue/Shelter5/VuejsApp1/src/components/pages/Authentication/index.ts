...
const WebEditorBackendApi = `http://${window.location.hostname}:8080`
...
export default {
    ...
    authenticate(context: Authentication, credentials: Credentials, redirect?: string) {
        Axios.post(`${WebEditorBackendApi}/api/v1/auth`, credentials)
            .then(({ data }) => {
    ...
    signup(context: Authentication, credentials: Credentials, redirect?: string) {
        Axios.post(`${WebEditorBackendApi}/api/v1/signup`, credentials)
            .then(({ data: { token } }) => {
